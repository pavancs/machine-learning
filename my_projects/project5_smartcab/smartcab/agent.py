...
class LearningAgent(Agent):
    ...
    def build_state(self):
        ...
        #remove None from waypoint
        
        state = (waypoint,)
        state = state + (inputs['light'],)
        state = state + (inputs['oncoming'],)
        state = state + (inputs['left'],)
    ...
    def createQ(self, state):
        ...
        #    if state not in self.Q.keys():
                #print state
                #print type(state)
        #        self.Q.update({state:{None:10, 'forward':10, 'left':10, 'right':10}})
                #self.Q.update({state:{None:0, 'forward':0, 'left':0, 'right':0}})
        
        if (self.learning) and (state not in self.Q):
            self.Q[state] = {action: 10.0 for action in self.valid_actions}
